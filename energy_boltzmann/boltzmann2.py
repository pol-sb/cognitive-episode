...
class find_molecule_percentage():
    ...
    def get_data(self):
        ...
        if len(file_list) > 0:
            ...
            log.e('ERROR - File error: Wrong file format (must be .txt) or \'boltzmann.py\' not in same folder as chosen text file. Exiting program...')
            quit()
    ...
    def lowest_energy_molecule(self, data_list):
        ...
        if min_names[0] != '':
            log.i('Minimum energy {} was found on molecule(s) {}.'.format(min_value, min_names))
        else:
